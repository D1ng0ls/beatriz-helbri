    ...
    const filteredPosts = posts
        .filter((post) => {
            if (selectedCategories?.length > 0) {
                return selectedCategories.includes(categoria[post.categoria_id]?.nome.normalize("NFD")
                .replace(/[\u0300-\u036f]/g, "")
            ...
            <div className="postagens">
                {filteredPosts.length > 0 ? (
                    filteredPosts.map((post, index) => (
                        <a
                            key={post.id}
                            href={`/post/${categoria[post.categoria_id]?.nome
                                .normalize("NFD")
                                .replace(/[\u0300-\u036f]/g, "")
                                .replace(/[^a-zA-Z0-9\s]/g, "")
                                .replace(/\s+/g, "-")
                                ...
                                .toLowerCase()}`}
                        >
                            <div className={`post post${index + 1}`}>
                                ...
                                <div className="text-post">
                                    <i>
                                        ...
                                        <span className="tag-post">
                                            {categoria[post.categoria_id]?.nome}
                                        </span>
                                    </i>
                                    <h2>{post.titulo}</h2>
                                    <p
                    ...
                    <div className="not-found">
                        <p>Nenhuma publicação encontrada <i className="bi-emoji-frown"></i></p>
